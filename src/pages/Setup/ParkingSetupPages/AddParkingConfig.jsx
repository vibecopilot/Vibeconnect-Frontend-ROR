...
const handleChange = async (e) => {
    ...
    const fetchParkingData = async () => {
      try {
        ...
        setTwoWheelerNonStack(
          data.filter((item) => item.vehicle_type === "2-wheeler" && !item.is_reserved)
        );
        setTwoWheelerReserved(
          data.filter((item) => item.vehicle_type === "2-wheeler" && item.is_reserved)
        );
        setFourWheelerNonStack(
          data.filter((item) => item.vehicle_type === "4-wheeler" && !item.is_reserved && !item.stack)
        );
        setFourWheelerStack(
          data.filter((item) => item.vehicle_type === "4-wheeler" && !item.is_reserved && item.stack)
        );
        setFourWheelerReserved(
          data.filter((item) => item.vehicle_type === "4-wheeler" && item.is_reserved)
        );
      } catch (error) {
  ...
  const saveParkingConfig = async () => {
    ...
    try {
      const response = await postParkingConfiguration({ all_parking: allParking });
          ...
          <div className="flex items-center">
            {editIndex === `NonStack-${index}` ? (
              ...
              <div className="flex items-center gap-2 w-full px-1">
                <span
                  onClick={() => handleEditStart(index, item.name, "NonStack")}
                  className="cursor-pointer text-gray-800 hover:text-blue-500 text-sm truncate"
                >
                  {item.name}
                </span>
                <button
                  onClick={() => deleteItem("Non Stack", "2-wheeler", index)}
          ...
          <div className="flex items-center">
            {editIndex === `Reserved-${index}` ? (
              ...
              <div className="flex items-center gap-2 w-full px-1">
                ...
                <button
                  onClick={() => deleteItem("Reserved", "2-wheeler", index)}
                  className="text-red-800 text-sm hover:text-red-600"
                >
                  X
                </button>
            ...
            {editIndex === `FourWheelerNonStack-${index}` ? (
              <div className="flex items-center">
                <input
                  ...
                  onChange={(e) => setEditName(e.target.value)}
                  onBlur={() =>
                    handleEditSave("Non Stack", "4-wheeler", index)
                  }
                  className="border px-1 py-0.5 rounded text-sm w-[40px] mr-1 focus:outline-none focus:ring-2 focus:ring-blue-400"
                />
                <button
              ...
              <div className="flex items-center gap-2 w-full px-1">
                <span
                  onClick={() =>
                    handleEditStart(index, item.name, "FourWheelerNonStack")
                  }
                  className="cursor-pointer text-gray-800 hover:text-blue-500 text-sm truncate"
                >
                  {item.name}
                ...
                <button
                  onClick={() => deleteItem("Non Stack", "4-wheeler", index)}
                  className="text-red-800 text-sm hover:text-red-600"
                >
                  X
                </button>
            ...
            {editIndex === `FourWheelerStack-${index}` ? (
              <div className="flex items-center">
                <input
                  ...
                  onChange={(e) => setEditName(e.target.value)}
                  onBlur={() =>
                    handleEditSave("Stack", "4-wheeler", index, true)
                  }
                  className="border px-1 py-0.5 rounded text-sm w-[40px] mr-1 focus:outline-none focus:ring-2 focus:ring-blue-400"
                />
                <button
              ...
              <div className="flex items-center gap-2 w-full px-1">
                <span
                  onClick={() =>
                    handleEditStart(index, item.name, "FourWheelerStack")
                  }
                  className="cursor-pointer text-gray-800 hover:text-blue-500 text-sm truncate"
                >
                  {item.name}
          ...
          <div className="flex items-center">
            {editIndex === `FourWheelerReserved-${index}` ? (
              ...
              <div className="flex items-center gap-2 w-full px-1">
                <span
                  onClick={() =>
                    handleEditStart(index, item.name, "FourWheelerReserved")
                  }
                  className="cursor-pointer text-gray-800 hover:text-blue-500 text-sm truncate"
                >
                  {item.name}
                ...
                <button
                  onClick={() => deleteItem("Reserved", "4-wheeler", index)}
                  className="text-red-800 text-sm hover:text-red-600"
                >
                  X
                </button>

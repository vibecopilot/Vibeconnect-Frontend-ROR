...
const EmployeeVisitor = () => {
  ...
  /* ================= FETCH EXPECTED VISITOR (PAGINATED) ================= */
  const fetchExpectedVisitor = async (page = 1) => {
    try {
      const visitorResp = await getExpectedVisitor(page, perPage);

      const sortedVisitor = visitorResp.data.visitors.sort((a, b) => {
      ...
      setCurrentPage(visitorResp.data.current_page);
      setTotalPages(visitorResp.data.total_pages);
      setTotalCount(visitorResp.data.total_count || 0);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  useEffect(() => {
    fetchExpectedVisitor(currentPage);
    fetchUserVisitors();
  }, [currentPage]);
  ...
  const handleCheckIn = async (id) => {
    ...
    try {
      await postVisitorCheckInCheckOut(id, payload);
      fetchExpectedVisitor(currentPage);
  ...
  const handleResendOtp = (row) => {
    ...
    row.isRotating = true;
    row.resendOtpTimer = 120;

    setVisitor((prevVisitor) =>
      prevVisitor.map((visitor) =>
        visitor.id === row.id
          ? { ...visitor, isRotating: true, resendOtpTimer: 120 }
          : visitor
      )
    );
    ...
    const intervalId = setInterval(() => {
      ...
      setVisitor((prevVisitor) =>
        prevVisitor.map((visitor) =>
          visitor.id === row.id
            ? { ...visitor, resendOtpTimer: row.resendOtpTimer }
            : visitor
        )
      );
  ...
  const VisitorColumns = [
    ...
    {
      name: "Host",
      selector: (row) => row?.hosts?.map((h) => <p key={h.id}>{h?.full_name}</p>),
    },
    {
      name: "Host Approval",
          ...
          return (
            <div className="flex items-center">
              ...
              {row.verified && row?.visitor_in_out === "IN" && (
                ...
                >
                  <FaPersonWalkingArrowLoopLeft /> OUT
                </button>
              )}
              {row?.verified && row?.visitor_in_out === "OUT" && (
                <p className="text-green-500 font-medium flex gap-2">
    ...
    },
    {
      ...
      selector: (row) => (
        <div
          className={`py-2 px-2 flex rounded-lg justify-center items-center ${
            row.isRotating ? "bg-red-700" : "bg-green-700"
          }`}
        >
          <FaRedo
            className={`text-white ${row.isRotating ? "rotate" : ""}`}
            onClick={() => handleResendOtp(row)}
          />
          {row.isRotating && (
            <span className="text-[12px] text-white">
              Resend in {row.resendOtpTimer}s
            </span>
          )}
        </div>
      ),
  ...
  const EmployeeVisitorColumns = [
    ...
    { name: "Visitor Type", selector: (row) => row.visit_type },
    { name: "Name", selector: (row) => row.name },
    { name: "Contact No.", selector: (row) => row.contact_no },
    { name: "Purpose", selector: (row) => row.purpose },
    { name: "Coming from", selector: (row) => row.coming_from },
    { name: "Expected Date", selector: (row) => row.expected_date },
    { name: "Expected Time", selector: (row) => row.expected_time },
    { name: "Vehicle No.", selector: (row) => row.vehicle_number },
    {
      name: "Host",
      selector: (row) =>
        row?.hosts?.map((h) => <p key={h.id}>{h?.host_details?.user_name}</p>),
    },
    {
      name: "Created by",
    ...
    <div className="visitors-page">
            ...
            <div className="flex gap-2 justify-between flex-col md:flex-row items-center">
              ...
              <div className="flex gap-2">
                {userType === "security_guard" && (
                  <button
                    className="bg-green-400 text-white rounded-md p-2 flex gap-2"
                ...
                <button onClick={handleAddNewVisitor}>
                  <div
                    ...
                    className="p-2 rounded-md text-white flex gap-2"
                  >
                    <PiPlusCircle /> Add New Visitor
                  </div>
                </button>
              </div>
            </div>
      ...
      {addNewVisitorModal && (
        <div className="fixed inset-0 flex justify-center items-center bg-black bg-opacity-30">
          <div className="bg-white p-4 rounded min-w-96">
            <label className="font-medium">Enter Mobile Number</label>
            <input
              type="tel"
              value={mobile}
            ...
            <div className="flex justify-center gap-2 mt-4">
              <button
                className="bg-red-400 text-white px-4 py-1"
              ...
              <Link
                ...
                className="bg-green-400 text-white px-4 py-1"
              >
                <FaCheck /> Submit
              </Link>
            </div>
      ...
      {otpModal && (
        <div className="fixed inset-0 flex justify-center items-center bg-black bg-opacity-30">
          <div className="bg-white p-4 rounded min-w-96">
            <h2 className="font-medium border-b mb-2">Verify OTP</h2>

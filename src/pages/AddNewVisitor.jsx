...
  
  // --- UPDATED: New state fields added for License and Consignment ---
...
    hostApproval: false,
    goodsInward: false,
    drivingLicense: false,         // NEW: Checkbox state for License
    consignmentForm: false,        // NEW: Checkbox state for Consignment
    host: "",
    passNumber: "",
    noOfGoods: "",
    goodsDescription: "",
    goodsAttachments: [],
    licenseAttachments: [],        // NEW: File state for License
    consignmentFormAttachment: [], // NEW: File state for Consignment
    supportCategory: "",
    slotNumber: "",
  })
  // -----------------------------------------------------------------
...
  
  // --- UPDATED: createNewVisitor with new fields and attachments ---
...
    formData.consignmentFormAttachment.forEach((file) => {
      postData.append(`visitor[consignment_form_attachment][]`, file, file.name)
    })
...
            </div>
          )}
{/* --- UPDATED CHECKBOX SECTION START --- */}
  ...
  <label className="flex items-center gap-2">
    <input
      ...
      onChange={() =>
        setFormData(prev => ({
          ...prev,
          hostApproval: !prev.hostApproval,
        }))
      }
    />
    <span>Skip Host Approval</span>
  </label>
  ...
  <label className="flex items-center gap-2">
    <input
      ...
      onChange={() =>
        setFormData(prev => ({
          ...prev,
          goodsInward: !prev.goodsInward,
        }))
      }
    />
    <span>Goods Inwards</span>
  </label>
...
{/* --- CONDITIONAL FIELDS SECTION START --- */}
...
{/* NEW: License Attachment Section */}
...
      isMulti={false} // Assuming a single driving license is uploaded
    />
  </>
)}

{/* NEW: Consignment Form Attachment Section */}
{formData.consignmentForm && (
  <>
    <div className="grid grid-cols-3 gap-2 my-2">
      <div className="flex flex-col gap-2">
        <p className="font-medium">No. of Goods :</p>
...
    </div>
    <FileInputBox
      handleChange={(files) => handleFileChange(files, "consignmentFormAttachment")}
      fieldName={"consignmentFormAttachment"}
      isMulti={false} // Assuming a single consignment form is uploaded
    />
  </>
)}
{/* --- CONDITIONAL FIELDS SECTION END --- */}

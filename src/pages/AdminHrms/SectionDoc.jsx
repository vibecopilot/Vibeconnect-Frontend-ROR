...
import { FaIdCard, FaCertificate, FaFileContract, FaIdBadge, FaFileAlt } from "react-icons/fa"
...
import { dateFormat, dateFormatSTD, dateTimeFormat } from "../../utils/dateUtils"
...
const SectionDoc = () => {
  ...
  const familyData ={
    ...
    try {
      console.log(familyData, familyMember)
      await postFamilyEsic(familyData)
      ...
      // Optionally, refresh the family members list here
      // fetchFamilyMembers();
    } catch (error) {
    ...
    <div className="flex flex-col ml-20">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {documentList.map((doc, index) => (
                      ...
                      >
                        ...
                        <h3 className="text-lg font-semibold text-gray-800 mb-2">{capitalize(doc.document_name)}</h3>
                        <p className="text-sm text-gray-500">Updated On: {dateFormat(doc.updated_date)}</p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {lettersList.map((letter, index) => (
                    ...
                    >
                      ...
                      <h3 className="text-lg font-semibold text-gray-800 mb-2">{capitalize(letter?.document_name)}</h3>
                      <p className="text-sm text-gray-500">Updated On: {dateFormat(letter?.updated_date)}</p>
                      <div className="flex justify-end w-full ">
                        {/* <button
                          className="border-blue-400 px-2 hover:bg-yellow-100 border rounded-l-md p-1"
              ...
              <div className="bg-green-100 p-4 rounded-lg mb-1">
                ...
                <div className="flex justify-center ">
                  {esic.map((letter, index) => (
                    <div key={letter.id} className="flex flex-col justify-center gap-4 w-full p-4">
          ...
          <Accordion
            ...
            content={
              <div className="bg-green-100 p-4 rounded-lg mb-1">
                ...
                {/* Add your family members list here */}
              </div>
            }
          />
        </div>
      ...
      >
        <div className="bg-white p-6 rounded-lg shadow-lg max-w-md w-full flex flex-col h-auto max-h-[80vh] overflow-y-auto">
          ...
          <h2 className="font-medium text-xl border-b border-gray-3">Add Document</h2>
          <div className="flex flex-col gap-3 mt-2 ">
            ...
            <div className="flex justify-end w-full gap-2">
              <button onClick={handleAddEmployeeDocs} className="bg-blue-400 text-white p-1 px-2 w-full rounded-md">
                Save
              </button>
              <button className="bg-red-400 text-white p-1 px-2 w-full rounded-md" onClick={closeDocs}>
                Close
              </button>
            </div>
          </div>
      ...
      >
        <div className="bg-white p-6 rounded-lg shadow-lg max-w-md w-full flex flex-col h-auto max-h-[80vh] overflow-y-auto">
          <h2 className="font-medium text-xl border-b border-gray-300 mb-4">Add Family Member</h2>
      ...
      >
        <div className="bg-white p-6 rounded-lg shadow-lg max-w-md w-full flex flex-col h-auto max-h-[80vh] overflow-y-auto">
          {/* <h2 className="text-2xl font-semibold mb-4">{currentItem?.name}</h2> */}
          <h2 className="font-medium text-xl border-b border-gray-3">Delete Document</h2>
          <div className="flex flex-col gap-2">
            <p className="font-medium mt-2">Are you sure you want to delete this document?</p>
            <div className="flex justify-end gap-2">
              <button className="rounded-full px-4 p-1 bg-green-400 text-white" onClick={handleDeleteEmployeeDoc}>
                Yes
              </button>
              <button className="rounded-full px-4 p-1 bg-red-400 text-white" onClick={() => setDeleteDocModal(false)}>
                No
              </button>
            </div>
          </div>
      ...
      >
        <div className="bg-white p-6 rounded-lg shadow-lg max-w-md w-full flex flex-col h-auto max-h-[80vh] overflow-y-auto">
          ...
          <h2 className="font-medium text-xl border-b border-gray-3">Delete Document</h2>
          <div className="flex flex-col gap-2">
            <p className="font-medium mt-2">Are you sure you want to delete this Letter?</p>
            <div className="flex justify-end gap-2">
              <button className="rounded-full px-4 p-1 bg-green-400 text-white" onClick={handleDeleteEmployeeLetter}>
                Yes
              </button>
              <button className="rounded-full px-4 p-1 bg-red-400 text-white" onClick={() => setDeleteLetter(false)}>
                No
              </button>
            </div>
          </div>

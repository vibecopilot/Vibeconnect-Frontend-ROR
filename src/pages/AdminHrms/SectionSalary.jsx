...
import { getEmployeeSalaryDetails ,getHrmsCtcTemplate ,createEmployeeProfile,getEmployeeDetails ,getCtcProfile , deleteCtcProfile ,getHrmsPayrollSlipDetails} from "../../api";
...
const fetchCtcProfiles = async () => {
  ...
  try {
    ...
    setCtcProfiles(Array.isArray(data) ? data : []);
  } catch (error) {
  ...
  const columns = [
      ...
      cell: (row) => (
        <div>
         <span>Date :- {new Date(row.created_date).toLocaleDateString()}</span> 
          <br />
          <span>Created Time :- {new Date(row.created_date).toLocaleTimeString()}</span>
    ...
    { 
      ...
      selector: (row) => {
        ...
        return template ? template.template_name : `Template ID: ${row.ctc_template}`;
      },
      sortable: true
    },
    {
      ...
      cell: (row) => (
        <div className="flex items-center gap-4">
          ...
          <button className="text-red-400" onClick={() => handleDeleteProfile(row.id)}>
            <FaTrash  size={15} />
          </button>
        </div>
...
const themeColor = useSelector((state)=> state.theme.color)
  ...
  return (
    ...
    <div className="flex flex-col ml-20">
      ...
      <div className="flex">
        ...
        <label htmlFor="" className="text-[16px] font-bold ">Select Ctc Template</label>
        <Select
    ...
    <div className="mt-5">
      <Table 
       ...
       data={ctcProfiles || []}
       isPagination={true}
       noDataComponent={<div className="p-4 text-center">No CTC profiles found</div>}
      />
    </div>
  )}
        ...
        {page === "add" && <AddNewCTC setPageChange={setPage} empId={id} fetchEmployeeSalary={fetchEmployeeSalary}  selectedTemplate={selectedTemplate?.data} fetchCtcProfiles={fetchCtcProfiles}/>}
  ...
  <div className="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50 z-50">
    <div className="bg-white p-8 rounded-lg shadow-lg w-2/4 max-h-screen overflow-y-auto">
      ...
      <div className="mb-4">
        <p>
          <strong>Template Name:</strong> {templateDetails.template_name}
        </p>
        <p>
          <strong>Created Date:</strong> {new Date(templateDetails.created_date).toLocaleDateString()}
        </p>
      </div>
      ...
      <div className="mb-4">
        ...
        {isTaxSettingsVisible && (
          <div className="ml-4">
            {templateDetails.deductions_details.map((deduction, index) => (
              <p key={index}>
                <strong>{deduction.name}:</strong> {deduction.value > 0 ? "Yes" : "No"}
                {deduction.percentage_of_salary > 0 && (
                  <span> ({deduction.percentage_of_salary}% of salary)</span>
                )}
              </p>
            ))}
          </div>
      ...
      <div className="mb-4">
        ...
        <p
          className="cursor-pointer"
          onClick={() => setCTCComponentsVisible(!isCTCComponentsVisible)}
        >
              ...
              {templateDetails.components_details
                ...
                .map((component, index) => (
                  <div key={index} className="flex justify-between">
                    <label className="text-gray-600">{component.name}</label>
      ...
      <div className="mb-4">
        <p
          className="cursor-pointer"
          onClick={() => setEmployerContributionsVisible(!isEmployerContributionsVisible)}
        >
          ...
          <div className="ml-4">
            <div className="mt-4 space-y-4 w-4/5">
              ...
              {templateDetails.deductions_details
                ...
                .map((deduction, index) => (
                  <div key={index} className="flex justify-between">
                    <label className="text-gray-600">{deduction.name}</label>
                    <div className="flex gap-8">
                      <p>{deduction.value}</p>
      ...
      <div className="mb-4">
        <p
          className="cursor-pointer"
          onClick={() => setEmployeeDeductionsVisible(!isEmployeeDeductionsVisible)}
        >
      ...
      <div>
        ...
        <div className="w-3/4">
          {/* Calculate totals */}
          {(() => {
            const fixedComponentsTotal = templateDetails.components_details
              .filter(comp => comp.component_type === "fixed")
              .reduce((sum, comp) => sum + comp.value, 0);
            
            const variableComponentsTotal = templateDetails.components_details
              .filter(comp => comp.component_type === "variable")
              .reduce((sum, comp) => sum + comp.value, 0);
            
            const employerDeductionsTotal = templateDetails.deductions_details
              .filter(ded => ded.deduction_type === "fixed")
              .reduce((sum, ded) => sum + ded.value, 0);
            
            const employeeDeductionsTotal = templateDetails.deductions_details
              .filter(ded => ded.deduction_type === "variable")
              .reduce((sum, ded) => sum + ded.value, 0);
            
            const grossSalary = fixedComponentsTotal + variableComponentsTotal;
            const takeHomeSalary = grossSalary - employeeDeductionsTotal;
            ...
            return (
              <>
                ...
                <div className="flex justify-between">
                  ...
                  <p className="ml-24">₹{(totalCTC + variableComponentsTotal).toFixed(2)}</p>
                  <p>₹{((totalCTC + variableComponentsTotal) * 12).toFixed(2)}</p>
                </div>
              </>
            );
          })()}

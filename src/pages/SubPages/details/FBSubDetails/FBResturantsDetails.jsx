...
const renderImages = (images, emptyText) => {
  if (!images?.length) {
    return <p>{emptyText}</p>;
  }

  return images.map((attachment, i) => (
    <div
      key={i}
      className="w-64 h-44 rounded-lg overflow-hidden shadow-md border cursor-pointer hover:scale-105 transition"
      onClick={() =>
        window.open(
  ...
  <div className="flex gap-4 flex-wrap">
    {renderImages(formData.cover_images,"No Cover Image")}
  </div>
</div>
        <div className="my-5 p-5">
<h3 className="border-b text-left text-xl border-black mb-6 font-bold">
...
<div className="flex gap-4 flex-wrap">
{renderImages(formData.menu_images,"No Menu Image")}
</div>
</div>
...
<div className="flex gap-4 flex-wrap">
{renderImages(formData.gallery_images,"No Gallery Image")}
</div>
</div>
      </div>

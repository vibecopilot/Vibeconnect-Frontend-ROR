            ...
            <div className="flex flex-wrap gap-8 items-center mt-6 border-t pt-4 border-gray-200">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className="text-sm px-3 py-1 rounded border border-gray-400"
                >
                  {consignmentFile ? "Change File" : "Upload Consignment"}
                </button>
                <input
                  type="file"
...
//           passNumber: editDetail.pass_number || "",
//           supportCategory: editDetail.visitor_staff_category_id || "",
//           slotNumber: editDetail.parking_slot_id || "", // NOTE: we now send parking_slot_id below
//           noOfGoods: editDetail.goods_inward_info?.no_of_goods || "",
//           goodsDescription: editDetail.goods_inward_info?.description || "",
//           notes: editDetail.notes || "",
//         }));
...
//   // helper for boolean formatting if backend expects "1"/"0"
//   const boolToInt = (b) => (b ? "1" : "0");
...
//     // booleans as "1"/"0" – adjust if your backend accepts "true"/"false"
//     postData.append(
//       "visitor[skip_host_approval]",
//       boolToInt(formData.hostApproval)
//     );
...
//     // enums normalized for backend – adjust if your backend expects different values
//     const apiVisitType =
//       selectedVisitorType === "Support Staff" ? "support_staff" : "guest";
//     const apiFrequency =
//       selectedFrequency === "Frequently" ? "frequently" : "once";
...
//     // IMPORTANT: use parking_slot_id to match read-side
//     postData.append("visitor[parking_slot_id]", formData.slotNumber);

//     // optionally send notes if backend supports it
//     postData.append("visitor[notes]", formData.notes || "");

//     // working_days – if backend uses day strings, this is fine.
//     // If it uses indexes, replace day with index.
//     selectedWeekdays.forEach((dayStr) => {
//       // send day string:
//       postData.append("visitor[working_days][]", dayStr);

//       // If backend expects indexes, use:
//       // const map = weekdaysMap.find((d) => d.day === dayStr);
//       // if (map) postData.append("visitor[working_days][]", map.index);
//     });

//     // extra visitors nested attributes
//     visitors.forEach((extraVisitor, index) => {
//       if (extraVisitor.id) {
//         postData.append(
//           `visitor[extra_visitors_attributes][${index}][id]`,
...
//     // goods fields – adjust keys to match your backend.
//     if (formData.goodsInward) {
//       // flat fields example:
//       postData.append("visitor[no_of_goods]", formData.noOfGoods || "");
//       postData.append(
...
//       // If backend uses nested attributes:
//       // postData.append("visitor[goods_inward_info_attributes][no_of_goods]", formData.noOfGoods || "");
//       // postData.append("visitor[goods_inward_info_attributes][description]", formData.goodsDescription || "");
//     }

...
const EditMasters = () => {
  ...
  useEffect(() => {
    const fetcMasterDetails = async () => {
      try {
        ...
        setInventoryType(data.inventory_type === 1 ? "spares" : "consumable");
        setcriticalvalue(data.criticality === 1 ? "critical" : "noncritical");

        if (data.asset_group_id) {
          await fetchSubGroups(data.asset_group_id);
        }
      } catch (error) {
        console.log(error);
      }
  ...
  const fetchSubGroups = async (groupId) => {
    try {
      ...
      const subGroups = response.data.map((item) => ({
        id: item.id,
        name: item.name,
      }));
      ...
      console.log("Subgroup fetch error:", error);
    }
  };
  ...
  const handleChange = async (e) => {
    ...
    if (name === "asset_group_id") {
      const groupId = Number(value);

      setFormData((prev) => ({
        ...prev,
        asset_group_id: groupId,
        asset_sub_group_id: "", // reset subgroup
      }));
  ...
  const handleMasters = async () => {
    if (!formData.asset_id) {
      toast.error("Please select Asset Name");
      return;
    }

    if (!formData.asset_group_id) {
      toast.error("Please select Asset Group");
      return;
    }

    if (!formData.asset_sub_group_id) {
      toast.error("Please select Asset Sub Group");
      return;
    }
    ...
    try {
      await UpdateMasters(sendData, id);
                ...
                <div className="flex flex-col">
                  ...
                  >
                    ...
                    {assetGroups.map((assetGroup) => (
                      <option value={Number(assetGroup.id)} key={assetGroup.id}>
                        {assetGroup.name}
                      </option>
                    ))}
                  </select>

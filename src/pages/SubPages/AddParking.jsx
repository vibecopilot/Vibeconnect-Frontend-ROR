...
import { getAssignedTo, getAvailableParkingNumber, getFloors, getParkingConfiguration, getParkingSlots, postParking } from "../../api";
...
const AddParking = () => {
  ...
  const handleSubmit = async () => {
    ...
    sendData.append("booking_parking[booking_start_time]", formData.booking_start_time);
    sendData.append("booking_parking[booking_end_time]", formData.booking_end_time);
...
<Navbar/>
      <div className="flex flex-col md:items-center mb-10 p-4 w-full">
        <div style={{background: themeColor}} className="flex justify-center bg-black mx-5 my-2 w-full p-2 rounded-md">
          <h2 className="text-xl font-semibold text-center mx-10 text-white w-full" style={{background: themeColor}}>
            Book Parking
          </h2>
        </div>
        ...
        <div className="md:grid flex flex-col grid-cols-3 items-center my-2">
          ...
          {behalf === "others" && (
              ...
              <label className="font-semibold" htmlFor="">Select User</label>
              <select 
              name="user_id"
    ...
    <option value={building.id} key={building.id}>
      {formatTime(building.start_hr, building.start_min)} to {formatTime(building.end_hr, building.end_min)}
    </option>
  ))}
              </select>
            </div>
            ...
            <div className="grid  items-center ">
                <label htmlFor="" className="font-semibold">Parking Number</label>
                <select name="parking_id" id="" 
